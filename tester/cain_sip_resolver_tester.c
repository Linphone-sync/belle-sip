...
static void reset_endpoint(endpoint_t *endpoint) {
	...
	if ((endpoint->query_type == DNS_T_A) || (endpoint->query_type == DNS_T_AAAA)) {
		cain_sip_list_for_each(endpoint->results_list, free_ai_result);
	} else if (endpoint->query_type == DNS_T_SRV) {
		cain_sip_list_for_each(endpoint->results_list, free_srv_result);
	}
...
static void srv_query(void) {
	...
	CU_ASSERT_PTR_NOT_NULL_FATAL(client);
	timeout = cain_sip_stack_get_dns_timeout(client->stack);
	client->resolver_id = cain_sip_resolve_srv(client->stack, SRV_DOMAIN, "udp", resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
	CU_ASSERT_TRUE(wait_for(client->stack, &client->resolve_done, 1, timeout));
	CU_ASSERT_PTR_NOT_EQUAL(client->results_list, NULL);
	CU_ASSERT_NOT_EQUAL(cain_sip_list_size(client->results_list), 0);
	if (client->results_list && (cain_sip_list_size(client->results_list) > 0)) {
		struct dns_srv *result_srv = cain_sip_list_nth_data(client->results_list, 0);
