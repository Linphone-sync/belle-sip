...
static void reset_endpoint(endpoint_t *endpoint) {
	...
	if (endpoint->srv_list != NULL) {
		cain_sip_list_for_each(endpoint->srv_list, free_srv_result);
...
static void ipv4_a_query(void) {
	...
	client->resolver_id = cain_sip_resolve(client->stack, IPV4_SIP_DOMAIN, SIP_PORT, AF_INET, a_resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
...
static void ipv4_a_query_no_result(void) {
	...
	CU_ASSERT_PTR_NOT_NULL_FATAL(client);
	timeout = cain_sip_stack_get_dns_timeout(client->stack);
	client->resolver_id = cain_sip_resolve(client->stack, IPV4_SIP_BAD_DOMAIN, SIP_PORT, AF_INET, a_resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
	CU_ASSERT_TRUE(wait_for(client->stack, &client->resolve_done, 1, timeout));
...
static void ipv4_a_query_timeout(void) {
	...
	client->resolver_id = cain_sip_resolve(client->stack, "toto.com", SIP_PORT, AF_INET, a_resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
...
static void ipv4_a_query_multiple_results(void) {
	...
	CU_ASSERT_PTR_NOT_NULL_FATAL(client);
	timeout = cain_sip_stack_get_dns_timeout(client->stack);
	client->resolver_id = cain_sip_resolve(client->stack, IPV4_MULTIRES_DOMAIN, SIP_PORT, AF_INET, a_resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
	CU_ASSERT_TRUE(wait_for(client->stack, &client->resolve_done, 1, timeout));
	CU_ASSERT_PTR_NOT_EQUAL(client->ai_list, NULL);
	if (client->ai_list) {
...
static void ipv6_aaaa_query(void) {
	...
	CU_ASSERT_PTR_NOT_NULL_FATAL(client);
	timeout = cain_sip_stack_get_dns_timeout(client->stack);
	client->resolver_id = cain_sip_resolve(client->stack, IPV6_SIP_DOMAIN, SIP_PORT, AF_INET6, a_resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
	CU_ASSERT_TRUE(wait_for(client->stack, &client->resolve_done, 1, timeout));
	CU_ASSERT_PTR_NOT_EQUAL(client->ai_list, NULL);
	if (client->ai_list) {
