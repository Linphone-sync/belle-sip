...
static void resolve(void) {
	...
	client->resolver_id = cain_sip_resolve(client->stack, peer_name, peer_port, family, resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	...
	if (client->result) {
		...
		ai = cain_sip_ip_address_to_addrinfo(IPV4_SIP_IP, peer_port);
		if (ai) {
	...
	family = AF_INET;
	peer_name = IPV4_SIP_DOMAIN;
	cain_sip_stack_set_send_error(client->stack, -1);
	client->resolver_id = cain_sip_resolve(client->stack, peer_name, peer_port, family, resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_EQUAL(client->resolver_id, 0);
	cain_sip_stack_set_send_error(client->stack, 0);

	/* IPv4 A query timeout */
	reset_endpoint(client);
	family = AF_INET;
	peer_name = IPV4_SIP_DOMAIN;
	cain_sip_stack_set_tx_delay(client->stack, timeout);
	client->resolver_id = cain_sip_resolve(client->stack, peer_name, peer_port, family, resolve_done, client, cain_sip_stack_get_main_loop(client->stack));
	CU_ASSERT_NOT_EQUAL(client->resolver_id, 0);
	CU_ASSERT_FALSE(wait_for(client->stack, &client->resolve_done, 1, timeout));
	CU_ASSERT_PTR_EQUAL(client->result, NULL);
	cain_sip_stack_set_tx_delay(client->stack, 0);
	...
	if (client->result) {
		...
		ai = cain_sip_ip_address_to_addrinfo(IPV6_SIP_IP, peer_port);
		if (ai) {
...
int cain_sip_resolver_test_suite(){
