...
static void test_simple_contact_header(void) {
	cain_sip_header_contact_t* L_tmp;
	...
	L_tmp = cain_sip_header_contact_parse(l_raw_header);
	L_contact = CAIN_SIP_HEADER_CONTACT(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);

	L_uri = cain_sip_header_address_get_uri((cain_sip_header_address_t*)L_contact);
...
static void test_complex_contact_header(void) {
	cain_sip_header_contact_t* L_contact;
	cain_sip_uri_t* L_uri;
	cain_sip_header_contact_t* L_tmp = cain_sip_header_contact_parse("Contact: \"jéremis\" <sip:sip.linphone.org>;expires=3600;q=0.7, sip:titi.com");
	cain_sip_header_t* l_next;
	cain_sip_header_contact_t* L_next_contact;
	char* l_raw_header;
	float l_qvalue;
	...
	l_qvalue = cain_sip_header_contact_get_qvalue(L_contact);
	CU_ASSERT_EQUAL(l_qvalue,0.7f);

	l_next = cain_sip_header_get_next(CAIN_SIP_HEADER(L_contact));
	L_next_contact = CAIN_SIP_HEADER_CONTACT(l_next);
	...
	l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_contact));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_contact));
...
static void test_from_header(void) {
	...
	L_tmp = cain_sip_header_from_parse(l_raw_header);
	L_from = CAIN_SIP_HEADER_FROM(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	...
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "titi.com");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_from_get_tag(L_from),"12345-abc");
	cain_sip_object_unref(L_from);
}

static void test_from_header_with_paramless_address_spec(void) {
	cain_sip_header_from_t* L_from = cain_sip_header_from_parse("From: sip:bob@titi.com;tag=dlfjklcn6545614XX");
	CU_ASSERT_PTR_NOT_NULL_FATAL(cain_sip_header_from_get_tag(L_from));
	CU_ASSERT_STRING_EQUAL(cain_sip_header_from_get_tag(L_from),"dlfjklcn6545614XX");
	cain_sip_object_unref(L_from);
}

static void test_to_header_with_paramless_address_spec(void) {
	cain_sip_header_to_t* L_to = cain_sip_header_to_parse("To: sip:bob@titi.com;tag=dlfjklcn6545614XX");
	CU_ASSERT_PTR_NOT_NULL_FATAL(cain_sip_header_to_get_tag(L_to));
	CU_ASSERT_STRING_EQUAL(cain_sip_header_to_get_tag(L_to),"dlfjklcn6545614XX");
	cain_sip_object_unref(L_to);
}

static void test_contact_header_with_paramless_address_spec(void) {
	cain_sip_header_contact_t* L_contact = cain_sip_header_contact_parse("Contact: sip:bob@titi.com;expires=60");
	CU_ASSERT_EQUAL(cain_sip_header_contact_get_expires(L_contact),60);
	cain_sip_object_unref(L_contact);
}
...
static void test_to_header(void) {
	...
	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_to));
	CU_ASSERT_PTR_NOT_NULL_FATAL(L_uri);
	CU_ASSERT_STRING_EQUAL(cain_sip_header_address_get_displayname(CAIN_SIP_HEADER_ADDRESS(L_to)), "super man");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_to_get_tag(L_to),"12345-abc");
	cain_sip_object_unref(L_to);
...
static void test_via_header(void) {
	...
	L_via = CAIN_SIP_HEADER_VIA(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	...
	CU_ASSERT_STRING_EQUAL(cain_sip_header_via_get_host(L_via), "::1");
	...
	l_next = cain_sip_header_get_next(CAIN_SIP_HEADER(L_via));
	L_next_via = CAIN_SIP_HEADER_VIA(l_next);
...
static void test_call_id_header(void) {
	cain_sip_header_call_id_t* L_tmp;
	...
	L_tmp= cain_sip_header_call_id_parse(l_raw_header);
	L_call_id = CAIN_SIP_HEADER_CALL_ID(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_cseq_header(void) {
	...
	L_tmp = cain_sip_header_cseq_parse(l_raw_header);
	L_cseq = CAIN_SIP_HEADER_CSEQ(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	...
	/*test factory*/
	L_cseq = cain_sip_header_cseq_create(1,"INFO");
	CU_ASSERT_EQUAL(cain_sip_header_cseq_get_seq_number(L_cseq),1);
	CU_ASSERT_STRING_EQUAL(cain_sip_header_cseq_get_method(L_cseq),"INFO");
	cain_sip_object_unref(L_cseq);
}

static void test_content_type_header(void) {
	cain_sip_header_content_type_t* L_tmp;
	...
	L_tmp = cain_sip_header_content_type_parse(l_raw_header);
	L_content_type = CAIN_SIP_HEADER_CONTENT_TYPE(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_record_route_header(void) {
	cain_sip_uri_t* L_uri;
	cain_sip_header_t* l_next;
	cain_sip_header_record_route_t* L_next_route;
	...
	cain_sip_free(l_raw_header);
	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_record_route));
	CU_ASSERT_PTR_NULL(cain_sip_uri_get_user(L_uri));
	...
	l_next = cain_sip_header_get_next(CAIN_SIP_HEADER(L_record_route));
	L_next_route = CAIN_SIP_HEADER_RECORD_ROUTE(l_next);
...
static void test_route_header(void) {
	cain_sip_header_route_t* L_route;
	cain_sip_uri_t* L_uri;
	cain_sip_header_t* l_next;
	cain_sip_header_route_t* L_next_route;
	...
	L_route = cain_sip_header_route_create(address);
	CU_ASSERT_PTR_NOT_NULL_FATAL(L_route);
	l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_route));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_route));
	L_route = cain_sip_header_route_parse(l_raw_header);
	cain_sip_free(l_raw_header);
	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_route));
	CU_ASSERT_PTR_NULL(cain_sip_uri_get_user(L_uri));
	...
	l_next = cain_sip_header_get_next(CAIN_SIP_HEADER(L_route));
	L_next_route = CAIN_SIP_HEADER_ROUTE(l_next);
	CU_ASSERT_PTR_NOT_NULL(L_next_route);
	CU_ASSERT_PTR_NOT_NULL( cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_next_route)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_route));
}
...
static void test_service_route_header(void) {
	cain_sip_header_service_route_t* L_service_route = cain_sip_header_service_route_parse("Service-Route: <sip:orig@scscf.ims.linphone.com:6060;lr>");
	cain_sip_uri_t* L_uri;
	char* l_raw_header;
	CU_ASSERT_PTR_NOT_NULL_FATAL(L_service_route);
	l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_service_route));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_service_route));
	L_service_route = cain_sip_header_service_route_parse(l_raw_header);
	cain_sip_free(l_raw_header);
	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_service_route));
	CU_ASSERT_PTR_NOT_NULL(cain_sip_uri_get_user(L_uri));
	CU_ASSERT_EQUAL(cain_sip_uri_get_port(L_uri), 6060);
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_service_route));
}

static void test_content_length_header(void) {
	cain_sip_header_content_length_t* L_tmp;
	...
	L_tmp = cain_sip_header_content_length_parse(l_raw_header);
	L_content_length = CAIN_SIP_HEADER_CONTENT_LENGTH(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_header_extension(void) {
	cain_sip_header_extension_t* L_tmp;
	...
	L_tmp = cain_sip_header_extension_parse(l_raw_header);
	L_extension = CAIN_SIP_HEADER_EXTENSION(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_authorization_header(void) {
	...
	cain_sip_header_authorization_t* L_tmp;
	...
	L_tmp = cain_sip_header_authorization_parse(l_raw_header);
	L_authorization = CAIN_SIP_HEADER_AUTHORIZATION(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void check_header_authenticate(cain_sip_header_www_authenticate_t* authenticate) {
	cain_sip_list_t* qop;
	CU_ASSERT_PTR_NOT_NULL(authenticate);
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_realm(authenticate), "atlanta.com");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_domain(authenticate), "sip:boxesbybob.com");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_nonce(authenticate), "c60f3082ee1212b402a21831ae");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_algorithm(authenticate), "MD5");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_opaque(authenticate), "1bc7f9097684320");

	CU_ASSERT_PTR_NOT_NULL_FATAL(qop=cain_sip_header_www_authenticate_get_qop(authenticate));
	...
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_qop(authenticate)->data, "auth");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_www_authenticate_get_scheme(authenticate), "Digest");
	CU_ASSERT_EQUAL(cain_sip_header_www_authenticate_is_stale(authenticate),1);
	cain_sip_object_unref(CAIN_SIP_OBJECT(authenticate));
}
...
static void test_www_authenticate_header(void) {
	...
	cain_sip_header_www_authenticate_t* L_tmp;
	cain_sip_header_www_authenticate_t* l_authenticate = cain_sip_header_www_authenticate_parse(l_header);


	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(l_authenticate));
	cain_sip_object_unref(CAIN_SIP_OBJECT(l_authenticate));
	L_tmp = cain_sip_header_www_authenticate_parse(l_raw_header);
	l_authenticate = CAIN_SIP_HEADER_WWW_AUTHENTICATE(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_proxy_authenticate_header(void) {
	const char* l_header = "Proxy-Authenticate: Digest "
			"algorithm=MD5, realm=\"atlanta.com\", opaque=\"1bc7f9097684320\","
			" qop=\"auth,auth-int\", nonce=\"c60f3082ee1212b402a21831ae\", stale=true, domain=\"sip:boxesbybob.com\"";
	cain_sip_header_proxy_authenticate_t* L_tmp;
	cain_sip_header_proxy_authenticate_t* L_proxy_authorization = cain_sip_header_proxy_authenticate_parse(l_header);
	//cain_sip_list_t* qop;

	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_proxy_authorization));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_proxy_authorization));
	L_tmp = cain_sip_header_proxy_authenticate_parse(l_raw_header);
	L_proxy_authorization = CAIN_SIP_HEADER_PROXY_AUTHENTICATE(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	check_header_authenticate(CAIN_SIP_HEADER_WWW_AUTHENTICATE(L_proxy_authorization));
}

static void test_max_forwards_header(void) {
	const char* l_header = "Max-Forwards: 6";
	cain_sip_header_max_forwards_t* L_tmp;
	...
	l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_max_forwards));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_max_forwards));
	L_tmp = cain_sip_header_max_forwards_parse(l_raw_header);
	L_max_forwards = CAIN_SIP_HEADER_MAX_FORWARDS(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
...
static void test_user_agent_header(void) {
	...
	cain_sip_header_user_agent_t* L_tmp;
	...
	L_tmp = cain_sip_header_user_agent_parse(l_raw_header);
	L_user_agent = CAIN_SIP_HEADER_USER_AGENT(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));

	cain_sip_free(l_raw_header);

	products = cain_sip_header_user_agent_get_products(L_user_agent);
...
static void test_expires_header(void) {
	cain_sip_header_expires_t* L_tmp;
	...
	L_tmp= cain_sip_header_expires_parse(l_raw_header);
	L_expires = CAIN_SIP_HEADER_EXPIRES(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	CU_ASSERT_EQUAL(cain_sip_header_expires_get_expires(L_expires), 3600);
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_expires));
	/*test factory*/
	L_expires = cain_sip_header_expires_create(600);
	CU_ASSERT_EQUAL(cain_sip_header_expires_get_expires(L_expires), 600);
	cain_sip_object_unref(L_expires);
}

static void test_allow_header(void) {
	cain_sip_header_allow_t* L_tmp;
	...
	L_tmp = cain_sip_header_allow_parse(l_raw_header);
	L_allow = CAIN_SIP_HEADER_ALLOW(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
...
static void test_address_with_error_header(void) {
	...
	laddress = cain_sip_header_address_parse("liblinphone_tester");
	CU_ASSERT_PTR_NULL(laddress);
}
...
static void test_address_header(void) {
	...
	CU_ASSERT_STRING_EQUAL("toto",cain_sip_header_address_get_displayname(laddress))
	L_uri = cain_sip_header_address_get_uri(laddress);

	CU_ASSERT_PTR_NOT_NULL(cain_sip_uri_get_user(L_uri));
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "81.56.11.2");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_user(L_uri), "liblinphone_tester");
	CU_ASSERT_EQUAL(cain_sip_uri_get_port(L_uri), 5060);
	cain_sip_object_unref(CAIN_SIP_OBJECT(laddress));
}
...
static void test_very_long_address_header(void) {
	...
	cain_sip_header_address_t* laddress = cain_sip_header_address_parse(raw);
	CU_ASSERT_PTR_NOT_NULL_FATAL(laddress);
	L_uri = cain_sip_header_address_get_uri(laddress);

	CU_ASSERT_PTR_NOT_NULL(cain_sip_uri_get_user(L_uri));
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "sip.linphone.org");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_user(L_uri), "jehan");
	CU_ASSERT_STRING_EQUAL(cain_sip_parameters_get_parameter(CAIN_SIP_PARAMETERS(L_uri),"pn-tok")
...
static void test_subscription_state_header(void) {
	cain_sip_header_subscription_state_t* L_tmp;
	cain_sip_header_subscription_state_t* L_subscription_state = cain_sip_header_subscription_state_parse("Subscription-State: terminated;expires=600");
	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_subscription_state));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_subscription_state));
	L_tmp = cain_sip_header_subscription_state_parse(l_raw_header);
	L_subscription_state = CAIN_SIP_HEADER_SUBSCRIPTION_STATE(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);

	CU_ASSERT_STRING_EQUAL(cain_sip_header_subscription_state_get_state(L_subscription_state), "terminated");
	CU_ASSERT_EQUAL(cain_sip_header_subscription_state_get_expires(L_subscription_state), 600);
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_subscription_state));
}

static void test_refer_to_header(void) {
	cain_sip_uri_t* L_uri;
	cain_sip_header_refer_to_t* L_refer_to = cain_sip_header_refer_to_parse("Refer-To: <sip:dave@denver.example.org?Replaces=12345%40192.168.118.3%3Bto-tag%3D12345%3Bfrom-tag%3D5FFE-3994>");
	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_refer_to));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_refer_to));
	L_refer_to = cain_sip_header_refer_to_parse(l_raw_header);
	cain_sip_free(l_raw_header);

	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_refer_to));

	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_user(L_uri),"dave");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "denver.example.org");
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_refer_to));
	/*test factory*/
	L_refer_to = cain_sip_header_refer_to_create(cain_sip_header_address_parse("\"super man\" <sip:titi.com>"));
	CU_ASSERT_PTR_NOT_NULL_FATAL(L_refer_to);
	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_refer_to));
	CU_ASSERT_PTR_NOT_NULL_FATAL(L_uri);
	CU_ASSERT_STRING_EQUAL(cain_sip_header_address_get_displayname(CAIN_SIP_HEADER_ADDRESS(L_refer_to)), "super man");
	cain_sip_object_unref(L_refer_to);

}

static void test_referred_by_header(void) {
	cain_sip_uri_t* L_uri;
	cain_sip_header_referred_by_t* L_referred_by = cain_sip_header_referred_by_parse("Referred-By: sip:r@ref.example;cid=\"2UWQFN309shb3@ref.example\"");
	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_referred_by));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_referred_by));
	L_referred_by = cain_sip_header_referred_by_parse(l_raw_header);
	cain_sip_free(l_raw_header);

	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_referred_by));

	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_user(L_uri),"r");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "ref.example");
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_referred_by));
}

static void test_replaces_header(void) {
	cain_sip_header_replaces_t* L_tmp;
	cain_sip_header_replaces_t* L_replaces = cain_sip_header_replaces_parse("Replaces: 12345@149.112.118.3;to-tag=12345;from-tag=54321");
	...
	CU_ASSERT_STRING_EQUAL(cain_sip_header_replaces_get_call_id(L_replaces), "12345@149.112.118.3");
	CU_ASSERT_STRING_EQUAL(cain_sip_header_replaces_get_from_tag(L_replaces), "54321");
...
static void test_replaces_escaped_header(void) {
	...
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));
	cain_sip_free(l_raw_header);
	cain_sip_free(escaped_to_string);
...
static void test_date_header(void){
	cain_sip_header_date_t *date,*date2;
	time_t utc;
#define DATE_EXAMPLE "Thu, 21 Feb 2002 13:02:03 GMT"
	date=cain_sip_header_date_parse("Date: " DATE_EXAMPLE);
	CU_ASSERT_PTR_NOT_NULL(date);
	utc=cain_sip_header_date_get_time(date);
	CU_ASSERT_TRUE(utc!=(time_t)-1);
	
	date2=cain_sip_header_date_create_from_time(&utc);
	CU_ASSERT_PTR_NOT_NULL(date2);
	CU_ASSERT_TRUE(strcmp(cain_sip_header_date_get_date(date2),DATE_EXAMPLE)==0);

}

static void test_p_preferred_identity_header(void) {
	cain_sip_uri_t* L_uri;
	cain_sip_header_p_preferred_identity_t* L_p_preferred_identity = cain_sip_header_p_preferred_identity_parse("P-Preferred-Identity: \"Cullen Jennings\" <sip:fluffy@cisco.com>");
	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_p_preferred_identity));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_p_preferred_identity));
	L_p_preferred_identity = cain_sip_header_p_preferred_identity_parse(l_raw_header);
	cain_sip_free(l_raw_header);

	L_uri = cain_sip_header_address_get_uri(CAIN_SIP_HEADER_ADDRESS(L_p_preferred_identity));
	CU_ASSERT_STRING_EQUAL(cain_sip_header_address_get_displayname(CAIN_SIP_HEADER_ADDRESS(L_p_preferred_identity)),"Cullen Jennings");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_user(L_uri),"fluffy");
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(L_uri), "cisco.com");
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_p_preferred_identity));
}
...
static void test_privacy(const char* raw_header,const char* values[],size_t number_values) {
	...
	cain_sip_header_privacy_t* L_tmp;
	cain_sip_header_privacy_t* L_privacy = cain_sip_header_privacy_parse(raw_header);
	char* l_raw_header = cain_sip_object_to_string(CAIN_SIP_OBJECT(L_privacy));
	int i=0;
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_privacy));
	L_tmp = cain_sip_header_privacy_parse(l_raw_header);
	L_privacy = CAIN_SIP_HEADER_PRIVACY(cain_sip_object_clone(CAIN_SIP_OBJECT(L_tmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(L_tmp));

	cain_sip_free(l_raw_header);

	list = cain_sip_header_privacy_get_privacy(L_privacy);
