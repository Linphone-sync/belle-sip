...
static void test_register_channel_inactive(void){
	cain_sip_listening_point_t *lp=cain_sip_provider_get_listening_point(prov,"TCP");
	CU_ASSERT_PTR_NOT_NULL_FATAL(lp);
	cain_sip_stack_set_inactive_transport_timeout(stack,5);
	cain_sip_listening_point_clean_channels(lp);
	CU_ASSERT_EQUAL(cain_sip_listening_point_get_channel_count(lp),0);
	register_test("tcp",1);
	CU_ASSERT_EQUAL(cain_sip_listening_point_get_channel_count(lp),1);
	cain_sip_stack_sleep(stack,5000);
	CU_ASSERT_EQUAL(cain_sip_listening_point_get_channel_count(lp),0);
	cain_sip_stack_set_inactive_transport_timeout(stack,3600);
}
