...
static void test_attribute_2(void) {
	cain_sdp_attribute_t* lTmp;
	cain_sdp_attribute_t* lAttribute = cain_sdp_attribute_parse("a=ice-pwd:31ec21eb38b2ec6d36e8dc7b\r\n");
	char* l_raw_attribute = cain_sip_object_to_string(CAIN_SIP_OBJECT(lAttribute));
	cain_sip_object_unref(CAIN_SIP_OBJECT(lAttribute));
	lTmp = cain_sdp_attribute_parse(l_raw_attribute);
	cain_sip_free(l_raw_attribute);
	lAttribute = CAIN_SDP_ATTRIBUTE(cain_sip_object_clone(CAIN_SIP_OBJECT(lTmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(lTmp));
	CU_ASSERT_STRING_EQUAL(cain_sdp_attribute_get_name(lAttribute), "ice-pwd");
	CU_ASSERT_STRING_EQUAL(cain_sdp_attribute_get_value(lAttribute), "31ec21eb38b2ec6d36e8dc7b");
	CU_ASSERT_TRUE(cain_sdp_attribute_as_value(lAttribute));
	cain_sip_object_unref(CAIN_SIP_OBJECT(lAttribute));
}
...
static void test_bandwidth(void) {
	...
	cain_sip_object_unref(CAIN_SIP_OBJECT(l_bandwidth));
	cain_sip_free(l_raw_bandwidth);
}

static void test_origin(void) {
	cain_sdp_origin_t* lTmp;
	cain_sdp_origin_t* lOrigin = cain_sdp_origin_parse("o=jehan-mac 3800 2558 IN IP4 192.168.0.165");
	char* l_raw_origin = cain_sip_object_to_string(CAIN_SIP_OBJECT(lOrigin));
	cain_sip_object_unref(CAIN_SIP_OBJECT(lOrigin));
	lTmp = cain_sdp_origin_parse(l_raw_origin);
	lOrigin = CAIN_SDP_ORIGIN(cain_sip_object_clone(CAIN_SIP_OBJECT(lTmp)));
	cain_sip_object_unref(CAIN_SIP_OBJECT(lTmp));
	CU_ASSERT_STRING_EQUAL(cain_sdp_origin_get_address(lOrigin), "192.168.0.165");
	CU_ASSERT_STRING_EQUAL(cain_sdp_origin_get_address_type(lOrigin), "IP4");
	CU_ASSERT_STRING_EQUAL(cain_sdp_origin_get_network_type(lOrigin), "IN");
	cain_sip_object_unref(CAIN_SIP_OBJECT(lOrigin));
	cain_sip_free(l_raw_origin);
}


static void test_malformed_origin(void) {
	cain_sdp_origin_t* lOrigin = cain_sdp_origin_parse("o=Jehan Monnier 3800 2558 IN IP4 192.168.0.165");
...
static void test_media(void) {
	...
	list = cain_sdp_media_get_media_formats(l_media);
...
static void test_media_description_base(cain_sdp_media_description_t* media_description) {
	const char* attr[] ={"99 MP4V-ES/90000"
				...
				,"98 H263-1998/90000"
				,"98 CIF=1;QCIF=1"};
	cain_sdp_connection_t* lConnection;
	...
	/*connection*/
	lConnection = cain_sdp_media_description_get_connection(l_media_description);
	CU_ASSERT_STRING_EQUAL(cain_sdp_connection_get_address(lConnection), "192.168.0.18");
	CU_ASSERT_STRING_EQUAL(cain_sdp_connection_get_address_type(lConnection), "IP4");
	CU_ASSERT_STRING_EQUAL(cain_sdp_connection_get_network_type(lConnection), "IN");
...
static void test_media_description(void) {
	...
	cain_sip_object_unref(CAIN_SIP_OBJECT(l_media_description));
	cain_sip_free(l_raw_media_description);
	return;
}
...
static void test_simple_session_description(void) {
	...
	cain_sip_free(l_raw_session_description);
	...
	l_origin = cain_sdp_session_description_get_origin(l_session_description);
	...
	media_descriptions = cain_sdp_session_description_get_media_descriptions(l_session_description);
	...
	cain_sip_object_unref(l_session_description);
	return;
}
...
static void test_session_description(void) {
	...
	CU_ASSERT_PTR_NOT_NULL(cain_sdp_session_description_get_session_name(l_session_description));
	CU_ASSERT_STRING_EQUAL(cain_sdp_session_name_get_value(cain_sdp_session_description_get_session_name(l_session_description)),"SIP Talk");
...
static void test_overflow(void){
	cain_sdp_session_description_t* sdp;
	cain_sip_list_t *mds;
	cain_sdp_media_description_t *vmd;
	int i;
	...
	mds=cain_sdp_session_description_get_media_descriptions(sdp);
	CU_ASSERT_PTR_NOT_NULL(mds);
	CU_ASSERT_PTR_NOT_NULL(mds->next);
	vmd=(cain_sdp_media_description_t*)mds->next->data;
	for(i=0;i<16;i++){
	...
	cain_sip_message("marshal size is %i",err);
...
static void test_mime_parameter(void) {
	...
	for (;mime_parameter_list_iterator!=NULL;mime_parameter_list_iterator=mime_parameter_list_iterator->next) {
		cain_sdp_media_description_append_values_from_mime_parameter(l_media_description,(cain_sdp_mime_parameter_t*)mime_parameter_list_iterator->data);
	}
	cain_sdp_media_description_set_attribute_value(l_media_description,"ptime","40");
	cain_sip_list_free_with_data(mime_parameter_list, (void (*)(void*))cain_sip_object_unref);

	mime_parameter_list = cain_sdp_media_description_build_mime_parameters(l_media_description);
	cain_sip_object_unref(l_media_description);
