...
static void check_uri_and_headers(cain_sip_message_t* message) {
	if (cain_sip_message_is_request(message)) {
		...
		CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"User-Agent"));
		CU_ASSERT_PTR_NOT_NULL(CAIN_SIP_HEADER_USER_AGENT(cain_sip_message_get_header(message,"User-Agent")));
	}
...
static void testRegisterMessage(void) {
	...
	char* encoded_message = cain_sip_object_to_string(CAIN_SIP_OBJECT(message));
	cain_sip_object_unref(CAIN_SIP_OBJECT(message));
	message = cain_sip_message_parse(encoded_message);
	...
	CU_ASSERT_PTR_NOT_NULL(CAIN_SIP_HEADER_EXPIRES(cain_sip_message_get_header(message,"Expires")));
	CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"Proxy-Authorization"));
	CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"Contact"));

	check_uri_and_headers(message);
	cain_sip_free(encoded_message);
...
static void testInviteMessage(void) {
	...
	CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"Contact"));
	CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"Authorization"));
	check_uri_and_headers(message);
	cain_sip_object_unref(message);
	cain_sip_free(encoded_message);
}
static void test401Response(void) {
	...
	cain_sip_object_unref(CAIN_SIP_OBJECT(message));
	message = cain_sip_message_parse(encoded_message);
	cain_sip_response_t* response = CAIN_SIP_RESPONSE(message);
	CU_ASSERT_EQUAL(cain_sip_response_get_status_code(response),401);
	CU_ASSERT_STRING_EQUAL(cain_sip_response_get_reason_phrase(response),"Unauthorized");
	CU_ASSERT_PTR_NOT_NULL(cain_sip_message_get_header(message,"WWW-Authenticate"));
	check_uri_and_headers(message);
	cain_sip_object_unref(message);
	cain_sip_free(encoded_message);
}
...
static void test_extract_source() {
	...
	cain_sip_message_t* message = cain_sip_message_parse(invite_1);
	cain_sip_request_t* request = CAIN_SIP_REQUEST(message);
	cain_sip_uri_t* source =cain_sip_request_extract_origin(request);
	CU_ASSERT_PTR_NOT_NULL(source);
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(source),"37.59.129.73");
	CU_ASSERT_EQUAL(cain_sip_uri_get_port(source),0);
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_transport_param(source),"tcp");
	cain_sip_object_unref(message);

	message = cain_sip_message_parse(invite_2);
	request = CAIN_SIP_REQUEST(message);
	source =cain_sip_request_extract_origin(request);
	CU_ASSERT_PTR_NOT_NULL(source);
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_host(source),"81.56.113.2");
	CU_ASSERT_EQUAL(cain_sip_uri_get_port(source),15060);
	CU_ASSERT_STRING_EQUAL(cain_sip_uri_get_transport_param(source),"udp");
	cain_sip_object_unref(message);
