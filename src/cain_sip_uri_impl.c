...
void cain_sip_uri_fix(cain_sip_uri_t *uri){
	const char *transport=cain_sip_uri_get_transport_param(uri);
	if (transport && strcasecmp(transport,"tls")==0){
		cain_sip_uri_set_secure(uri,TRUE);
	}
}
...
static int uri_strncmp_common(const char*a,const char*b,size_t n,int case_sensitive) {
	...
	while (a[index_a]!='\0'&&b[index_b]!='\0'&&index_a<n&&index_b<n) {
		index_a+=cain_sip_get_char(a+index_a,n-index_a,&char_a);
		index_b+=cain_sip_get_char(b+index_b,n-index_b,&char_b);
...
#define PARAM_CASE_CMP(uri_a,uri_b,param) \
		a_param=cain_sip_parameters_get_case_parameter((cain_sip_parameters_t*) uri_a,param); \
		b_param=cain_sip_parameters_get_case_parameter((cain_sip_parameters_t*) uri_b,param);\
		if (a_param && b_param) { \
