...
static struct dns_hosts *hosts(cain_sip_resolver_context_t *ctx) {
	...
	if (ctx->stack->dns_user_hosts_file) {
		...
		if (error) {
			cain_sip_error("%s dns_hosts_loadfile(\"%s\"): %s for %s", __FUNCTION__, dns_strerror(error),ctx->stack->dns_user_hosts_file);
		}
	}
		...
		while (dns_rr_grep(&rr, 1, I, ans, &error)) {
			if (rr.section == section) {
				...
				if ((ctx->type == DNS_T_AAAA) && (rr.class == DNS_C_IN) && (rr.type == DNS_T_AAAA)) {
					...
					ctx->srv_list = cain_sip_list_insert_sorted(ctx->srv_list, res, srv_compare_prio);
