...
CAIN_SIP_INSTANCIATE_VPTR(cain_sip_sender_task_t,cain_sip_object_t,cain_sip_sender_task_uninit,NULL,NULL);
...
static void sender_task_res_done(void *data, const char *name, struct addrinfo *res){
	...
	if (res){
		...
		ev.port=t->hop.port;
		ev.host=t->hop.host;
		CAIN_SIP_PROVIDER_INVOKE_LISTENERS(t->provider,process_io_error,&ev);
		t->cb(t,t->cb_data,-1);
	}
}

void cain_sip_sender_task_send(cain_sip_sender_task_t *t, cain_sip_message_t *msg){
	cain_sip_stack_t *stack=cain_sip_provider_get_sip_stack(t->provider);
