...
user            :	  ( unreserved  | user_unreserved )+ {cain_sip_uri_set_user($an_sip_uri::current,(const char *)$text->chars);};
user_unreserved :  '&' | '=' | '+' | '$' | ',' | ';' | '?' | '/';
password        :	  ( unreserved  |'&' | '=' | '+' | '$' | ',' )*;
hostport        :	  host ( ':' port )?;
host            :	  hostname {cain_sip_uri_set_host($an_sip_uri::current,(const char *)$text->chars);};
