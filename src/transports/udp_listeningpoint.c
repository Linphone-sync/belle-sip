...
static cain_sip_socket_t create_udp_socket(const char *addr, int port, int *family){
...
static int cain_sip_udp_listening_point_init_socket(cain_sip_udp_listening_point_t *lp){
	lp->sock=create_udp_socket(cain_sip_uri_get_host(((cain_sip_listening_point_t*)lp)->listening_uri)
					,cain_sip_uri_get_port(((cain_sip_listening_point_t*)lp)->listening_uri),&lp->base.ai_family);
...
static void cain_sip_udp_listening_point_init(cain_sip_udp_listening_point_t *lp, cain_sip_stack_t *s, const char *ipaddress, int port) {
...
static int on_udp_data(cain_sip_udp_listening_point_t *lp, unsigned int events){
	...
	if (events & CAIN_SIP_EVENT_READ){
		...
		if (err==-1){
			char *tmp=cain_sip_object_to_string((cain_sip_object_t*) ((cain_sip_listening_point_t*)lp)->listening_uri);
			cain_sip_error("udp_listening_point: recvfrom() failed on [%s], : [%s] reopening server socket"
					,tmp
			...
			cain_sip_address_remove_v4_mapping((struct sockaddr*)&addr,(struct sockaddr*)&addr,&addrlen);
			...
			if (chan){
				/*notify the channel*/
				cain_sip_debug("Notifying udp channel, local [%s:%i]  remote [%s:%i]"
						,chan->local_ip
						,chan->local_port
						,chan->peer_name
						,chan->peer_port);
