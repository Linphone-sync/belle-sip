...
void cain_sip_main_loop_iterate(cain_sip_main_loop_t *ml){
	...
	cain_sip_pollfd_t *pfd=(cain_sip_pollfd_t*)cain_sip_malloc0(pfd_size);
	int i=0;
	cain_sip_source_t *s;
	...
	int ret;
	uint64_t cur;
	cain_sip_list_t *to_be_notified=NULL;
	...
	for(elem=ml->sources;elem!=NULL;elem=elem->next){
		...
		if (!s->cancelled){
			...
			if (revents!=0 || (s->timeout>=0 && cur>=s->expire_ms)){
				to_be_notified=cain_sip_list_append(to_be_notified,cain_sip_object_ref(s));
				s->expired=TRUE;
				if (s->revents==0)
					s->revents=CAIN_SIP_EVENT_TIMEOUT;
			}
		}else to_be_notified=cain_sip_list_append(to_be_notified,cain_sip_object_ref(s));
	}
	...
	for(elem=to_be_notified;elem!=NULL;){
		...
		if (!s->cancelled){
			...
			}else if (s->revents==CAIN_SIP_EVENT_TIMEOUT){
		...
		cain_sip_object_unref(s);
		cain_sip_free(elem); /*free just the element*/
		elem=next;
	}
