...
:         {IS_TOKEN(m)}?alpha_num EQUAL 
          media_value {cain_sdp_media_set_value($media::current,(const char*)$media_value.text->chars);} 
          ...
          SPACE proto {cain_sdp_media_set_media_protocol($media::current,(const char*)$proto.text->chars);}
          (SPACE fmt)?;
