...
@init {$header_call_id::current = cain_sip_header_call_id_new(); $ret=$header_call_id::current; }
  :  call_id_token /*( 'Call-ID' | 'i' )*/ hcolon call_id{cain_sip_header_call_id_set_call_id($header_call_id::current,(const char*) $call_id.text->chars); };
catch [ANTLR3_MISMATCHED_TOKEN_EXCEPTION]
{
   cain_sip_message("[\%s]  reason [\%s]",(const char*)EXCEPTION->name,(const char*)EXCEPTION->message);
...
header_address returns [cain_sip_header_address_t* ret]   
  : header_address_base[cain_sip_header_address_new()] {$ret=$header_address_base.ret;}; 

header_address_base[cain_sip_header_address_t* obj]      returns [cain_sip_header_address_t* ret]   
@init { $ret=obj; }
	   : name_addr[CAIN_SIP_HEADER_ADDRESS($ret)] 
	   | addr_spec[CAIN_SIP_HEADER_ADDRESS($ret)];
catch [ANTLR3_MISMATCHED_TOKEN_EXCEPTION]
{
  cain_sip_message("[\%s]  reason [\%s]",(const char*)EXCEPTION->name,(const char*)EXCEPTION->message);
...
header_date  returns [cain_sip_header_date_t* ret]     
scope { cain_sip_header_date_t* current; }
@init {$header_date::current = cain_sip_header_date_new(); $ret=$header_date::current; }
  :  date_token /*( 'Date' )*/ hcolon sip_date{cain_sip_header_date_set_date($header_date::current,(const char*) $sip_date.text->chars); };
catch [ANTLR3_MISMATCHED_TOKEN_EXCEPTION]
{
   cain_sip_message("[\%s]  reason [\%s]",(const char*)EXCEPTION->name,(const char*)EXCEPTION->message);
   cain_sip_object_unref($header_date::current);
   $ret=NULL;
}
...
header_refer_to  returns [cain_sip_header_refer_to_t* ret]   
  :   refer_to_token /*'Refer-To'*/ 
      hcolon 
      refer_to_spec[CAIN_SIP_HEADER_ADDRESS(cain_sip_header_refer_to_new())] {$ret = CAIN_SIP_HEADER_REFER_TO($refer_to_spec.ret);};

referred_by_token:  {IS_TOKEN(Referred-By)}? token;
header_referred_by  returns [cain_sip_header_referred_by_t* ret]   
  :   referred_by_token /*'Referred-By'*/ 
      hcolon 
      refer_to_spec[CAIN_SIP_HEADER_ADDRESS(cain_sip_header_referred_by_new())] {$ret = CAIN_SIP_HEADER_REFERRED_BY($refer_to_spec.ret);};
  
refer_to_spec [cain_sip_header_address_t* address]    returns [cain_sip_header_address_t* ret]
  :   (( name_addr[address] | paramless_addr_spec[address] )  ( semi generic_param [CAIN_SIP_PARAMETERS(address)] )* ){$ret=address;};
catch [ANTLR3_MISMATCHED_TOKEN_EXCEPTION]
{
   cain_sip_message("[\%s]  reason [\%s]",(const char*)EXCEPTION->name,(const char*)EXCEPTION->message);
   cain_sip_object_unref(address);
   $ret=NULL;
} 
...
@init { if ($header_via::current == NULL) {
            ...
            header = CAIN_SIP_HEADER($header_via::current); 
...
via_params        
	:	  /*via_ttl | via_maddr
                     |  via_branch
                     | via_extension */ via_received[$header_via::current] | generic_param [CAIN_SIP_PARAMETERS($header_via::current)];
/*via_ttl           
	:	  'ttl' EQUAL ttl;
via_maddr         
	:	  'maddr' EQUAL host;*/
via_received [cain_sip_header_via_t* object] 
  : {IS_TOKEN(received)}? token EQUAL via_address {cain_sip_header_via_set_received(object,(const char*)$via_address.text->chars);};
...
sent_by           
	:	  host {cain_sip_header_via_set_host($header_via::current,$host.ret);}
...
*/    
header_replaces  returns [cain_sip_header_replaces_t* ret] 
scope { cain_sip_header_replaces_t* current; }
@init { $header_replaces::current = cain_sip_header_replaces_new();$ret = $header_replaces::current; } 
 : {IS_TOKEN(Replaces)}? token /*"Replaces"*/ 
 hcolon call_id {cain_sip_header_replaces_set_call_id($header_replaces::current,(const char*)$call_id.text->chars);} 
  (semi  generic_param [CAIN_SIP_PARAMETERS($header_replaces::current)])* ;
catch [ANTLR3_MISMATCHED_TOKEN_EXCEPTION]
{
   cain_sip_message("[\%s]  reason [\%s]",(const char*)EXCEPTION->name,(const char*)EXCEPTION->message);
   cain_sip_object_unref($header_replaces::current);
   $ret=NULL;
}
...
header_extension[ANTLR3_BOOLEAN check_for_known_header]  returns [cain_sip_header_t* ret]
	:	   header_name 
	     ...
	     header_value {if (check_for_known_header && strcasecmp("Contact",(const char*)$header_name.text->chars) == 0) {
                    ...
                    }else if (check_for_known_header && strcasecmp(CAIN_SIP_REFERRED_BY,(const char*)$header_name.text->chars) == 0) {
...
@init {$hostport::current=uri;}
        :	  host ( COLON port {cain_sip_uri_set_port($hostport::current,$port.ret);})? {cain_sip_uri_set_host($hostport::current,$host.ret);};
