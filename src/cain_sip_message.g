...
qvalue         
	:	  ( '0' ( '.' DIGIT+)? )
                  | ( '1'( '.'DIGIT+)? );
*/
generic_param  returns [cain_sip_param_pair_t* ret]
	:	  SP* token ( SP* EQUAL SP* is_gen=gen_value )? {$ret=cain_sip_param_pair_new($token.text->chars,$is_gen.text?$gen_value.text->chars:NULL);};
gen_value      
	:	  token |  quoted_string;
...
contact_param  
	:	  (name_addr[(cain_sip_header_address_t*) $header_contact::current] 
	   | addr_spec[(cain_sip_header_address_t*) $header_contact::current]) (SEMI contact_params)*;
...
addr_spec[cain_sip_header_address_t* object]      
  :  uri {cain_sip_header_address_set_uri(object,cain_sip_uri_ref($uri.ret));};//| absoluteURI;

display_name[cain_sip_header_address_t* object]   
  :  token {cain_sip_header_address_set_displayname(object,$token.text->chars);}
     | quoted_string {cain_sip_header_address_set_quoted_displayname(object,$quoted_string.text->chars);}
     ;
...
contact_extension  
	:	  generic_param {cain_sip_param_pair_t* pair = $generic_param.ret ;
	                       if (strcmp("expires",pair->name) == 0) {
										        cain_sip_header_contact_set_expires($header_contact::current,atoi(pair->value));
										      } else if (strcmp("q",pair->name) == 0) {
										        cain_sip_header_contact_set_qvalue($header_contact::current,atof(pair->value));
										      } else {
										        cain_sip_warning("unknown contact param \%s",(const char *)$contact_extension.text->chars);
										      }
										      cain_sip_param_pair_unref(pair);
	                  };
/*
delta_seconds      
	:	  DIGIT+;*/
...
@init { $header_from::current = cain_sip_header_from_new(); }
...
from_spec   
	:	  ( name_addr[$header_from::current] | addr_spec[$header_from::current] )
               ( SEMI from_param )*;
from_param  
	:	  /*tag_param |*/ generic_param {cain_sip_param_pair_t* pair = $generic_param.ret ;
							                         if (strcmp("tag",pair->name) == 0) {
							                            cain_sip_header_from_set_tag($header_contact::current,pair->value);
							                          } else {
							                            cain_sip_warning("unknown from param \%s",(const char *)$from_param.text->chars);
							                          }
                                    cain_sip_param_pair_unref(pair);
                                    }
                       ;
...
hostport        :	  host ( COLON port )? {cain_sip_uri_set_host($uri::current,(const char *)$host.text->chars);};
...
port	:	DIGIT+ {cain_sip_uri_set_port($uri::current,atoi((const char *)$text->chars));};
...
other_param       :  pname ( EQUAL pvalue )?
  {
    if (strcmp("lr",(const char *)$pname.text->chars) == 0) {
      cain_sip_uri_set_lr_param($uri::current,1);
      } else if (strcmp("transport",(const char*)$pname.text->chars)==0) {
        cain_sip_uri_set_transport_param($uri::current,(const char *)$pvalue.text->chars);
      } else if (strcmp("user",(const char *)$pname.text->chars)==0) {
        cain_sip_uri_set_user_param($uri::current,(const char *)$pvalue.text->chars);
      } else if (strcmp("maddr",(const char *)$pname.text->chars)==0) {
        cain_sip_uri_set_maddr_param($uri::current,(const char *)$pvalue.text->chars);
      } else if (strcmp("ttl",(const char *)$pname.text->chars)==0) {
        cain_sip_uri_set_ttl_param($uri::current,atoi((const char *)$pvalue.text->chars));
      } else if (strcmp("method",(const char *)$pname.text->chars)==0) {
        cain_sip_uri_set_method_param($uri::current,(const char *)$pvalue.text->chars);
      } else {
        cain_sip_warning("unknown uri param \%s",(const char *)$other_param.text->chars);
      }
  };
pname             
	:	  paramchar+;
