...
dig_resp          
	:	  username { cain_sip_header_authorization_set_username($header_authorization::current,$username.ret);
	               cain_sip_free($username.ret);
	             }
	| realm { cain_sip_header_authorization_set_realm($header_authorization::current,$realm.ret);
                 cain_sip_free($realm.ret);
           } 
	| nonce { cain_sip_header_authorization_set_nonce($header_authorization::current,$nonce.ret);
                 cain_sip_free($nonce.ret);
           }
	| digest_uri
  | dresponse  { cain_sip_header_authorization_set_response($header_authorization::current,$dresponse.ret);
                 cain_sip_free($dresponse.ret);
               }
  /*| algorithm  {
                cain_sip_header_authorization_set_algoritm($header_authorization::current,$algorithm.ret);
               } 
  | cnonce
                      | opaque | message_qop
...
digest_uri        
	...
	{cain_sip_header_authorization_set_uri($header_authorization::current,$uri.ret);
