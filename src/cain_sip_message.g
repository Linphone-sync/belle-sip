...
@init {$response::current = cain_sip_response_new(); $ret=$response::current; }         
	:	  status_line message_header[CAIN_SIP_MESSAGE($response::current)]+ last_crlf=CRLF {*($message_raw::message_length)=0;} /*message_body*/ ;

status_line     
	:	  sip_version 
	   LWS status_code {cain_sip_response_set_status_code($response::current,atoi($status_code.text->chars));}
	   LWS reason_phrase {cain_sip_response_set_reason_phrase($response::current,$reason_phrase.text->chars);}
	   CRLF ;
...
dig_resp  [cain_sip_header_authorization_t* header_authorization_base]         
	...
	| realm { cain_sip_header_authorization_set_realm(header_authorization_base,$realm.ret);
                 cain_sip_free($realm.ret);
           } 
	| nonce { cain_sip_header_authorization_set_nonce(header_authorization_base,$nonce.ret);
                 cain_sip_free($nonce.ret);
           }
	| digest_uri[header_authorization_base]
  | dresponse  { cain_sip_header_authorization_set_response(header_authorization_base,$dresponse.ret);
                 cain_sip_free($dresponse.ret);
               }
  | algorithm  {
                cain_sip_header_authorization_set_algorithm(header_authorization_base,$algorithm.ret);
               } 
  | cnonce{
            cain_sip_header_authorization_set_cnonce(header_authorization_base,$cnonce.ret);
            cain_sip_free($cnonce.ret);
           }
  | opaque {
            cain_sip_header_authorization_set_opaque(header_authorization_base,$opaque.ret);
  ...
  | nonce_count{
            cain_sip_header_authorization_set_nonce_count(header_authorization_base,atoi($nonce_count.ret));
           } 
...
username returns [char* ret]          
	:	  username_token /*'username'*/ equal username_value {
                      $ret = _cain_sip_str_dup_and_unquote_string($username_value.text->chars);
                       };
...
cnonce returns [char* ret]            
	:	  {IS_TOKEN(cnonce)}? token /*'cnonce'*/ equal cnonce_value {
                                              $ret = _cain_sip_str_dup_and_unquote_string($cnonce_value.text->chars);
                                              };
cnonce_value      
	:	  nonce_value;
...
dresponse  returns [char* ret]       
	:	  {IS_TOKEN(response)}? token /*'response'*/ equal request_digest{
                      $ret = _cain_sip_str_dup_and_unquote_string($request_digest.text->chars);
                     };
request_digest    
...
@init { if ($header_contact::current == NULL) {
            ...
            cain_sip_header_set_next(CAIN_SIP_HEADER($header_contact::current),cain_sip_header_contact_new());
            $header_contact::current = cain_sip_header_get_next(CAIN_SIP_HEADER($header_contact::current));
         } 
      }
...
other_challenge [cain_sip_header_www_authenticate_t* www_authenticate]    
	:	  auth_scheme {cain_sip_header_www_authenticate_set_scheme(www_authenticate,$auth_scheme.text->chars);} 
	   LWS auth_param[NULL]
                       (comma auth_param[NULL])*;
digest_cln [cain_sip_header_www_authenticate_t* www_authenticate]         
	:	
  | realm {cain_sip_header_www_authenticate_set_realm(www_authenticate,$realm.ret);
           cain_sip_free($realm.ret);} 
  | nonce {cain_sip_header_www_authenticate_set_nonce(www_authenticate,$nonce.ret);
           cain_sip_free($nonce.ret);}
...
realm returns [char* ret]              
	:	  {IS_TOKEN(realm)}? token /*'realm'*/ equal realm_value {
	                    $ret = _cain_sip_str_dup_and_unquote_string($realm_value.text->chars);
	                     };
realm_value         
	:	  quoted_string ;
...
domain returns [char* ret]              
	:	  {IS_TOKEN(domain)}? token /*'domain'*/ equal quoted_string {
                      $ret = _cain_sip_str_dup_and_unquote_string($quoted_string.text->chars);};
	/* LDQUOT uri
                       ( SP+ uri )* RDQUOT;
uri                 
	:	  absoluteURI | '/'.;
...
nonce  returns [char* ret]             
	:	  {IS_TOKEN(nonce)}? token /*'nonce'*/ equal nonce_value{
                      $ret = _cain_sip_str_dup_and_unquote_string($nonce_value.text->chars);
                       };
...
@init { if ($header_record_route::current == NULL) {
            ...
            cain_sip_header_set_next(header,$header_record_route::current = cain_sip_header_record_route_new());
         } 
      }     
...
@init { if ($header_route::current == NULL) {
            ...
            cain_sip_header_set_next(header,$header_route::current = cain_sip_header_route_new());
         } 
      }      
...
@init { if ($header_via::current == NULL) {
            ...
            cain_sip_header_set_next(header,$header_via::current = cain_sip_header_via_new());
         } 
      }          
	...
	:	   header_name 
	     ...
	     header_value {if (check_for_known_header && strcmp("Contact",(const char*)$header_name.text->chars) == 0) {
                     ...
                     $ret = CAIN_SIP_HEADER(cain_sip_header_www_authenticate_parse((const char*)$header_extension.text->chars));
                      ...
                      cain_sip_header_extension_set_value($ret,(const char*)$header_value.text->chars);
