...
generic_param  returns [cain_sip_param_pair_t* ret]
	:	  token ( EQUAL is_gen=gen_value )? {$ret=cain_sip_param_pair_new((const char*)$token.text->chars,$is_gen.text?(const char*)$gen_value.text->chars:NULL);};
gen_value      
	:	  token |  quoted_string;
...
addr_spec      :  uri {cain_sip_header_address_set_uri((cain_sip_header_address_t*) $header_contact::current
                                                        ,cain_sip_uri_ref($uri.ret));
                      };//| absoluteURI;
display_name   :  token  | quoted_string {cain_sip_header_address_set_displayname((cain_sip_header_address_t*) $header_contact::current
                                                        , (const char*)$display_name.text->chars);};
