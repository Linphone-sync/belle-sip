...
cain_sip_error_code cain_sip_snprintf(char *buff, size_t buff_size, size_t *offset, const char *fmt, ...) {
	...
	if ((ret < 0)
		|| (ret >= (buff_size - *offset))) {
			error = CAIN_SIP_BUFFER_OVERFLOW;
		*offset = buff_size;
	} else {
		*offset += ret;
	}
...
#define DEFAULT_CLOCK_MODE CLOCK_REALTIME /*monotonic clock stop during sleep mode*/
	...
	if (clock_id==-1) {
		if (clock_gettime(DEFAULT_CLOCK_MODE,&ts)!=1){
			...
			cain_sip_fatal("Cannot find suitable clock mode");
		}
	}
	return clock_id;
}
uint64_t cain_sip_time_ms(void){
	...
	if (clock_gettime(find_best_clock_id(),&ts)==-1){
		cain_sip_error("clock_gettime() error for clock_id=%i: %s",find_best_clock_id(),strerror(errno));
		return 0;
	}
...
char* cain_sip_to_unescaped_string(const char* buff) {
	char output_buff[CAIN_SIP_MAX_TO_STRING_SIZE];
	unsigned int i;
	unsigned int out_buff_index=0;
	output_buff[CAIN_SIP_MAX_TO_STRING_SIZE-1]='\0';
	for(i=0;buff[i]!='\0' &&i<CAIN_SIP_MAX_TO_STRING_SIZE /*to make sure last param can be stored in escaped form*/;) {
		i+=cain_sip_get_char(buff+i,3,output_buff+out_buff_index++);
	}
...
char* cain_sip_to_escaped_string(const char* buff) {
	...
	unsigned int out_buff_index=0;
	output_buff[CAIN_SIP_MAX_TO_STRING_SIZE-1]='\0';
	for(i=0;buff[i]!='\0' &&i<CAIN_SIP_MAX_TO_STRING_SIZE-4 /*to make sure last param can be stored in escaped form*/;i++) {
