...
static void cain_sip_channel_destroy(cain_sip_channel_t *obj){
	...
	if (obj->resolver_id>0) cain_sip_resolve_cancel(cain_sip_stack_get_main_loop(obj->stack),obj->resolver_id);
	if (obj->inactivity_timer){
		cain_sip_main_loop_remove_source(obj->stack->ml,obj->inactivity_timer);
		cain_sip_object_unref(obj->inactivity_timer);
	}
...
static void update_inactivity_timer(cain_sip_channel_t *obj, int from_recv){
	int inactive_timeout=cain_sip_stack_get_inactive_transport_timeout(obj->stack)*1000;
	if (inactive_timeout>0){
		if (!obj->inactivity_timer ){
			obj->inactivity_timer=cain_sip_main_loop_create_timeout(obj->stack->ml,channel_inactive_timeout,obj,inactive_timeout,"Channel inactivity timer");
		}else{
			/*restart the timer for new period*/
			cain_sip_source_set_timeout(obj->inactivity_timer,inactive_timeout);
		}
	}else{
		if (obj->inactivity_timer){
			cain_sip_main_loop_remove_source(obj->stack->ml,obj->inactivity_timer);
	...
	if (from_recv)
		obj->last_recv_time=cain_sip_time_ms();
}
...
int cain_sip_channel_notify_timeout(cain_sip_channel_t *obj){
	const int too_long=60;
	if (cain_sip_time_ms() - obj->last_recv_time>=(too_long * 1000)){
		cain_sip_message("A timeout related to this channel occured and no message received during last %i seconds. This channel is suspect, moving to error state",too_long);
		channel_set_state(obj,CAIN_SIP_CHANNEL_ERROR);
		return TRUE;
	}
	return FALSE;
...
static void channel_process_queue(cain_sip_channel_t *obj){
	...
	while((msg=channel_pop_outgoing(obj))!=NULL && obj->state==CAIN_SIP_CHANNEL_READY) {
...
void cain_sip_channel_set_ready(cain_sip_channel_t *obj, const struct sockaddr *addr, socklen_t slen){
	...
	if (obj->local_ip==NULL){
		...
		if (err!=0){
			cain_sip_error("cain_sip_channel_set_ready(): getnameinfo() failed: %s",gai_strerror(err));
		}else{
...
void cain_sip_channel_resolve(cain_sip_channel_t *obj){
	...
	obj->resolver_id=cain_sip_resolve_a(obj->stack, obj->peer_name, obj->peer_port, obj->lp->ai_family, channel_res_done, obj, obj->stack->ml);
